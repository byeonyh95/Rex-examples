...
float input[400 * 600 * 3];
uint8_t iout[400 * 600];
uint8_t rout[400 * 600 * 3];
...
void ssd_post(float *decom_r_low, float *decom_i_low, int w, int h, ssd_ctx *ctx)
{
	int ret;

	//FILE *r = fopen("r.raw","w");
	//FILE *i = fopen("i.raw","w");

	for(int i=0; i<400 * 600 * 3; i++){
		//printf("%d\n",*pr);
		rout[i] = (uint8_t)(decom_r_low[i] * 255.0);
		if(rout[i] < 0) rout[i] = 0;
		if(rout[i] > 255) rout[i] = 255;

	}
	for(int j=0; j<400 * 600; j++){
		iout[j] = (uint8_t)(decom_i_low[j] * 255.0);
		if(rout[j] < 0) rout[j] = 0;
		if(rout[j] > 255) rout[j] = 255;
	}
	
	//fwrite(rout, sizeof(uint8_t), 400 * 600 * 3, r);	
	//fwrite(iout, sizeof(uint8_t), 400 * 600, i);
