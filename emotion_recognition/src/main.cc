...
#define MODEL_NAME	"/emotion.rknn" //Emotion-Recognition rknn model path
...
{	
	...
	while(1){
		...
		rga_set_rect(&r_dst.rect, 0, 0, 48, 48, 48, 48, RK_FORMAT_RGB_888); //Emotion-Recognition image input size (48, 48, 1)
		ret = c_RkRgaBlit(&r_src, &r_dst, NULL);

		ssd_run(&ctx, (uint8_t*)dst_v, 48, 48, 48*48*3); // 48 * 48 * 1 = 2304
	}
